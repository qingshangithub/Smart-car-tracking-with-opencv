...
class Car(object):
    ...
    def _stream_video(self):
        while self._keep_streaming:
            ...
            frame = cv2.resize(frame, (0, 0), fx=25, fy=25, interpolation=cv2.INTER_LINEAR)
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.JOYAXISMOTION:
                    speed = event.value if abs(event.value) >= 0.05 else 0
    ...
    def collect_data(self, fn):
        ...
        def record(self_):
            ...
            while self_.keep_serving:
                ...
                larger = cv2.resize(frame, (0, 0), fx=25, fy=25, interpolation=cv2.INTER_LINEAR)
